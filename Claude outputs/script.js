...
discordBtn.addEventListener("click", async () => {
  ...
  discordStatus.textContent = ok ? "Kopiert" : "Nicht kopiert";
  discordStatus.classList.toggle("is-done", ok);
  clearTimeout(copyTimer);
  copyTimer = setTimeout(() => {
    discordStatus.textContent = "Kopieren";
...
const STATUS_LABEL = { online: "Online", idle: "Abwesend", dnd: "Nicht stören", offline: "Offline" };
...
async function loadPresence() {
  ...
  try {
    ...
    detail.textContent = data.listening_to_spotify && data.spotify
      ? `Hört: ${data.spotify.song} – ${data.spotify.artist}`
      : "Playlists & Profil";
